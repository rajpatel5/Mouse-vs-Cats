...
int train_1layer_net(double sample[INPUTS],int label,double (*sigmoid)(double input), double weights_io[INPUTS][OUTPUTS])
{
  ...
  ***********************************************************************************************************/
	...
	// Initialize max and activations
	int i = 0;
    while (i < 10){
		max[i] = 0.0;
        activations[i] = 0.0;
	...
	// Siimulate feedforward & backprop, then check for the max
...
int classify_1layer(double sample[INPUTS],int label,double (*sigmoid)(double input), double weights_io[INPUTS][OUTPUTS])
{
  ...
  ***********************************************************************************************************/
	...
	while (i < OUTPUTS) {
		// Set new max
		if (activations[max] <= activations[i]) {
			max = i;
		}
	...
	return max;			// <---	This should return the class for this sample
}
  ...
  */ 
   ...
   ******************************************************************************************************/
	...
	while (i < OUTPUTS){
		...
		// Calcaluate sum
		while (j < INPUTS){
			sum = (weights_io[j][i] * sample[j]) + sum;
			j++;
		}
		
		// Store activations
		activations[i] = sigmoid(SIGMOID_SCALE * sum);
		i++;
	}
}
...
void backprop_1layer(double sample[INPUTS], double activations[OUTPUTS], double (*sigmoid)(double input), int label, double weights_io[INPUTS][OUTPUTS])
{
   ...
   */
    ...
    * ************************************************************************************************/
	...
	set_targets(sigmoid, target, &use_logistic, label);
	
	// Find error for each output and update weights
	i = 0;
	while (i < OUTPUTS) {
		...
		j = 0;
		while (j < INPUTS) {
			weights_io[j][i] = weights_io[j][i] + (ALPHA * error * sample[j]);
			j++;
		}
...
int train_2layer_net(double sample[INPUTS],int label,double (*sigmoid)(double input), int units, double weights_ih[INPUTS][MAX_HIDDEN], double weights_ho[MAX_HIDDEN][OUTPUTS])
{
  ...
  ***********************************************************************************************************/
	...
	double max[10];
    double activations[10];
    double simulate_H[units];
	
	int current = 0; // Number with most hits
    ...
    // Get the activations into simulate_H and update weights
    feedforward_2layer(sample, sigmoid, weights_ih, weights_ho, simulate_H, activations, units);
    backprop_2layer(sample, simulate_H, activations, sigmoid, label, weights_ih, weights_ho, units);
	
	// Simulate feedforward and backprop, then check for max
	current = classify_2layer(sample, label, sigmoid, units, weights_ih, weights_ho);
	return current;		// <--- This should return the class for this sample
}
...
int classify_2layer(double sample[INPUTS],int label,double (*sigmoid)(double input), int units, double weights_ih[INPUTS][MAX_HIDDEN], double weights_ho[MAX_HIDDEN][OUTPUTS])
{
  ...
  ***********************************************************************************************************/
	...
	while (i < OUTPUTS) {
		// Set new max
		if (activations[i] > current_max) {
			current_max = activations[i];
            max_index = i;
		}
	...
	return max_index;		// <--- Should return the class for this sample  
}
...
void feedforward_2layer(double sample[INPUTS], double (*sigmoid)(double input), double weights_ih[INPUTS][MAX_HIDDEN], double weights_ho[MAX_HIDDEN][OUTPUTS], double h_activations[MAX_HIDDEN],double activations[OUTPUTS], int units)
{
  ...
  */ 
   ...
   **************************************************************************************************/
	...
	// Feedforward for sample into hidden layers (Input layer)
	i = 0;
	while (i < units){ 
		sum = 0;
		...
		j = 0;
		while (j < INPUTS){
			sum = (weights_ih[j][i] * sample[j]) + sum;
			j++;
		}
		
		// Set activation
		h_activations[i] = sigmoid(SIGMOID_SCALE * sum);
		i++;
	}
	
	// For each output find activation  (Output layer)
	i = 0;
	while (i < OUTPUTS) {
		sum = 0;
		...
		j = 0;
		while (j < units){
			sum = (weights_ho[j][i] * h_activations[j]) + sum;
			j++;
		}
		
		// Set activation
		activations[i] = sigmoid(SIGMOID_SCALE * sum * (MAX_HIDDEN / units));
		i++;
	}
}
...
void backprop_2layer(double sample[INPUTS],double h_activations[MAX_HIDDEN], double activations[OUTPUTS], double (*sigmoid)(double input), int label, double weights_ih[INPUTS][MAX_HIDDEN], double weights_ho[MAX_HIDDEN][OUTPUTS], int units)
{
   ...
   */
    ...
    * ************************************************************************************************/
	...
	// Declare variables
	double target[OUTPUTS];
	double previous_weights[units][OUTPUTS];
    double d_error[OUTPUTS];
	double error = 0.0;
    int use_logistic = 0;
	int i, j, k;
	
    //Determine which sigmoid function we are using
	set_targets(sigmoid, target, &use_logistic, label);
	...
	while (i < OUTPUTS){
		j = 0;
        while (j < units){     
            // Get the output for logistic
            previous_weights[j][i] = weights_ho[j][i];		// Store original weights
            
            if (use_logistic == 1){ 
				// Logistic function
                d_error[i] = (target[i] - activations[i]) * (activations[i] * (1.0 - activations[i]));
                weights_ho[j][i] = (ALPHA * (d_error[i] * (h_activations[j]))) + weights_ho[j][i];
            }
            else{ 
				// Tanh function
                d_error[i] = (target[i] - activations[i]) * (1.0 - (activations[i] * activations[i]));
                weights_ho[j][i] = (ALPHA * (d_error[i] * h_activations[j])) + weights_ho[j][i];
            }
    ...
    while (i < units){
        ...
        while (j < INPUTS){     
			...
			k = 0;
            while (k < OUTPUTS){
                error = (previous_weights[i][k] * d_error[k]) + error; 
				k++;
            }
            ...
            if (use_logistic != 1){
				// Tanh function
				weights_ih[j][i] += (sample[j] * ALPHA * error) * (1.0 - (h_activations[i] * h_activations[i]));
            }
            else{ 
				// Logisitic function
                weights_ih[j][i] += (sample[j] * ALPHA * error) * (h_activations[i] * (1.0 - h_activations[i]));
            }
...
void set_targets(double (*sigmoid)(double input), double target[OUTPUTS], int *use_logistic, int label) {
	/* Initialze the target outputs */
	
	// Declare variables
	int i;
	
	// Set target for each output
	i = 0;
	while (i < OUTPUTS) {
		if (logistic != sigmoid) { 
			// Not logistic function
			target[i] = 0.6;
			if (i != label) {
				target[i] = -0.6;
			}
